...
### Language Modes (LostAndLucky overlay shape, Phase 6+)

Default agent prompts are in `agents/{name}/prompt.md` (English). Non-English markets use **overlays** under `locales/{lang}/agents/{name}/prompt.md` — only the prompt diff lives in the locale; the contract / runners / examples / evals are shared with canonical English.

**Resolution rule** (used by every agent + workflow load):

1. If `language.modes_dir` is set in `me/profile.yml` (Phase 7) / `config/profile.yml` (today) — e.g. `locales/de`:
   - Try `{language.modes_dir}/agents/{agent}/prompt.md` — load if exists.
   - Else fall back to `agents/{agent}/prompt.md` (canonical English).
2. Otherwise, load `agents/{agent}/prompt.md`.

Same shape for workflows: `{language.modes_dir}/workflows/{workflow}/definition.md` overlays `workflows/{workflow}/definition.md`. `{language.modes_dir}/_shared.md` is a full replacement of canonical `modes/_shared.md` for that locale.

**Locales currently shipping** (see `locales/CONTEXT.md` for the full coverage matrix):

| Locale | Path | Coverage |
|---|---|---|
| **de** | `locales/de/` | evaluator + apply-helper + pipeline-drain + _shared (DACH — Probezeit, AGG, 13. Monatsgehalt) |
| **fr** | `locales/fr/` | evaluator + apply-helper + pipeline-drain + _shared (FR/BE/CH/LU — CDI, SYNTEC, RTT) |
| **ja** | `locales/ja/` | evaluator + apply-helper + pipeline-drain + _shared (Japan — 正社員, 賞与, 36協定) |
| **ru** | `locales/ru/` | evaluator + apply-helper + interview-coach + pipeline-drain + _shared |
| **tr** | `locales/tr/` | evaluator + apply-helper + pipeline-drain + _shared (Turkey — SGK, kıdem tazminatı, AGİ) |
| **pt** | `locales/pt/` | evaluator + apply-helper + pipeline-drain + _shared (Brazil / Portugal) |

**When to use a locale:**

1. User explicitly says "use German modes" / "use French modes" / etc. → read from `locales/{lang}/` for this conversation.
2. User sets `language.modes_dir: locales/{lang}` in their profile → always use that locale.
3. AI detects the JD is in a non-English language and a matching locale exists → **suggest** switching, but user preference always wins.

**When NOT to:** If the user applies to English-language roles (even at non-English companies), use canonical English — unless user has explicitly requested another locale in this conversation, OR `language.modes_dir` is set in profile.

<!-- Legacy modes/{de,fr,ja,ru,tr,pt} folders are now empty (content moved in Phase 6 to locales/). Phase 7 removes them from update-system.mjs SYSTEM_PATHS. The legacy mode-list below was the pre-restructure documentation; preserved as a redirect note for any AI loading it from cache. -->

- **German (DACH market):** `modes/de/` (legacy path) — now empty; load from `locales/de/` instead.
- **French (Francophone market):** `modes/fr/` (legacy path) — now empty; load from `locales/fr/`.
- **Japanese (Japan market):** `modes/ja/` (legacy path) — now empty; load from `locales/ja/`.
- **Turkish (Turkey market):** `modes/tr/` (legacy path) — now empty; load from `locales/tr/`.
- **Russian (Russian market):** `locales/ru/` (full coverage including interview-coach).
- **Portuguese (BR/PT market):** `locales/pt/`.
